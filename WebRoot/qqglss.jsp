...
String qqr=request.getParameter("qqr");
String starttime=request.getParameter("starttime");
String endtime=request.getParameter("endtime");
String sql1="select  count(*) as total from request";
if(jjcd!=""){
sql1=sql1.concat(" where emergency='"+Tools.toChinese(jjcd)+"'");
}
if(qqr!=""){
if(jjcd!=""){
sql1=sql1.concat(" and reqname='"+Tools.toChinese(qqr)+"'");
}
else{sql1=sql1.concat(" where reqname='"+Tools.toChinese(qqr)+"'");}
}
if(starttime!=""&&endtime!="")
{
if(!(qqr==""&&jjcd=="")){sql1=sql1.concat(" and reqtime between '"+starttime+"' and '"+endtime+"'");}
else{sql1=sql1.concat("where reqtime  between '"+starttime+"' and '"+endtime+"'");}
}
...
if(jjcd!=""){
sql=sql.concat(" where emergency='"+Tools.toChinese(jjcd)+"'");
}
if(qqr!=""){
if(jjcd!=""){
sql=sql.concat(" and reqname='"+Tools.toChinese(qqr)+"'");
}
else{sql=sql.concat(" where reqname='"+Tools.toChinese(qqr)+"'");}
}
if(starttime!=""&&endtime!="")
{
if(!(qqr==""&&jjcd=="")){sql=sql.concat(" and reqtime between '"+starttime+"' and '"+endtime+"'");}
else{sql=sql.concat("where reqtime between '"+starttime+"' and '"+endtime+"'");}
}
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" style="table-layout:fixed">
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#a8c7ce"  style="table-layout:fixed">
    <tr><td class="STYLE23">&nbsp;&nbsp;&nbsp;紧急程度:<select name="jjcd" style="width:60px;"><option value="普通">普通</option><option value="紧急">紧急</option></select>&nbsp;&nbsp;请求人：<input name="qqr" type="text" style="width:90px;"/>&nbsp;&nbsp;请求时间段：<input name="starttime" type="date"/>—<input name="endtime" type="date"/>&nbsp;<input type="button" onClick="qqglss();" value="筛选" width="20" height="15" /></td></tr>
    </table>
  <tr>
